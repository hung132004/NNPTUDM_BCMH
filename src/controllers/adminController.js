...
async function updateOrderStatus(req, res, next) {
  try {
    ...
    await createNotification(order.user._id || order.user, {
      ...
      message: `ÄÆ¡n hÃ ng ${order._id} Ä‘Ã£ Ä‘Æ°á»£c chuyá»ƒn sang tráº¡ng thÃ¡i ${order.status}.`,
...
async function getInvoices(_req, res, next) {
  try {
...
async function getWarranties(_req, res, next) {
  try {
...
async function updateWarrantyStatus(req, res, next) {
  try {
